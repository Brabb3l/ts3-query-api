...
impl Decode for Permission {
    fn decode(decoder: &mut Decoder) -> Result<Permission, ParseError> {
        let id: String = decoder.advance_or_err("permsid")?;
        let value: i32 = decoder.advance_or_err("permvalue")?;

        Ok(Self::parse(&id, value, false).unwrap())
    }
}

impl EncodeList for Permission {
...
mod test {
    ...
    #[test]
    fn test_parse_bool() {
        let perm = Permission::parse("b_serverinstance_help_view", 1, true).unwrap();
    ...
    fn test_parse_unknown_error() {
        let perm = Permission::parse("unknown", 1, true);
